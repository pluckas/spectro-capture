...
class Guider:
    ...
    def _handle_event(self, ev: dict[str, Any]):
        ...
        if e == "AppState":
            ...
            with self.lock:
                ...
                # --- [Spectro Capture Custom Additions: Star metrics] ---
                # Capture star brightness and quality indicators from each GuideStep
                self.StarMass = ev.get("StarMass", self.StarMass)
                self.SNR = ev.get("SNR", self.SNR)
                self.HFD = ev.get("HFD", self.HFD)
                # ---------------------------------------------------------
    ...
    def SetExposure(self, seconds: float):
        """
        Set the guider camera exposure in seconds.
        Calls PHD2's 'set_exposure' RPC (expects milliseconds).
        """
        self._CheckConnected()
        try:
            self.Call("set_exposure", int(seconds * 1000))
        except Exception as e:
            raise GuiderError(f"set_exposure failed: {e}")
    ...
    # ------------------------------------------------------------
    #  Added convenience wrappers for additional PHD2 RPC calls
    # ------------------------------------------------------------
    
    def GetLockPosition(self) -> tuple[float, float]:
        """
        Return the current guide-star lock position (x, y).
        https://github.com/OpenPHDGuiding/phd2/wiki/EventMonitoring#get_lock_position
        """
        self._CheckConnected()
        res = self.Call("get_lock_position")
        xy = res.get("result")
        if not isinstance(xy, (list, tuple)) or len(xy) != 2:
            raise GuiderError(f"Unexpected lock position result: {xy!r}")
        return float(xy[0]), float(xy[1])
    
    def SetLockPosition(self, x: float, y: float):
        """
        Move the guide-star lock position to (x, y) pixels.
        https://github.com/OpenPHDGuiding/phd2/wiki/EventMonitoring#set_lock_position
        """
        self._CheckConnected()
        return self.Call("set_lock_position", [x, y])
    
    def FindStar(self, roi: list[int] | None = None):
        """
        Ask PHD2 to auto-select the brightest star.
        roi – optional list [x, y, width, height]
        https://github.com/OpenPHDGuiding/phd2/wiki/EventMonitoring#find_star
        """
        self._CheckConnected()
        params = [roi] if roi else []
        return self.Call("find_star", params)
    ...
    def GetStarPosition(self) -> tuple[float, float] | None:
        """
        Return the current guide star (x, y) position from PHD2,
        or None if not available.
        """
        try:
            res = self.Call("get_star_image_pos")
            xy = res.get("result")
            if not isinstance(xy, (list, tuple)) or len(xy) != 2:
                return None
            return float(xy[0]), float(xy[1])
    ...
    # --- ROI ADDITION (UTILITY FUNCTION) START -----------------------------------
    def is_star_in_roi(star_pos, roi_center, roi_size):
        """
        Check whether the detected star position lies within a rectangular ROI.
    
        Parameters
        ----------
        star_pos : tuple (x, y)
            The (x, y) coordinates of the detected guide star, in pixels.
        roi_center : tuple (x, y)
            The (x, y) center of the ROI region, in pixels.
        roi_size : int or float
            The full side length of the ROI box in pixels.
    
        Returns
        -------
        bool
            True if the star is inside the ROI, False otherwise.
        """
        if not star_pos or not roi_center:
            return False
        ...
        inside_x = (cx - half) <= star_x <= (cx + half)
        inside_y = (cy - half) <= star_y <= (cy + half)
    
        return inside_x and inside_y
    # --- ROI ADDITION (UTILITY FUNCTION) END -------------------------------------
